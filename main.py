...
@app.route('/rz', methods=['POST'])
def process_payment():
    try:
        # Get card data from request
        data = request.get_json()
        if not data or 'cc' not in data:
            return jsonify({"error": "Missing 'cc' parameter. Format: cc=number|mm|yy|cvv"}), 400
        ...
        token_create = base64.b64encode(json.dumps([{"name": "sardine", "metadata": {"session_id": coid_local}}], 
        ...
        if payment_id:
            ...
            result["final_response"] = json.loads(resp_final.text)
        else:
            result["error"] = json.loads(resp_create.text)
